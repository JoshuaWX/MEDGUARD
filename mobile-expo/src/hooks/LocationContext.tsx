...
export const LocationProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  const { user, initialized, sessionTrust } = useAuth();
  ...
  const confirmLocation = useCallback(async (point: LocationData) => {
    const id = activeUserId.current; if (!id || sessionTrust !== 'verified' || !locationSharingEnabled || !isOnline) return;
  ...
  }, [isOnline, locationSharingEnabled, sessionTrust]);
  ...
  const setLocationSharing = useCallback(async (enabled: boolean) => {
    const id = activeUserId.current; if (!id || sessionTrust !== 'verified') return false;
  ...
  }, [backgroundLocationEnabled, clearLocationState, refreshLocation, requestPermission, sessionTrust, startWatching]);
  const setBackgroundLocationEnabled = useCallback(async (enabled: boolean): Promise<BackgroundLocationStartResult> => {
    if (!activeUserId.current || sessionTrust !== 'verified' || (enabled && !locationSharingEnabled)) return { ok: false, reason: 'start_failed' };
    if (!enabled) {
    ...
    return started;
  }, [locationSharingEnabled, sessionTrust]);
  const setManualAlertState = useCallback(async (state: string) => {
    if (sessionTrust !== 'verified') return false;
  ...
  }, [clearLocationState, refreshLocation, sessionTrust, startWatching, stopWatching, user?.id]);
